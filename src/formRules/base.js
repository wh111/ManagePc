...
let baseRules = {
    ...
    numberSection(min = 0, max = 999999999) {
        return (rule, value = 0, callback) => {
            (isNaN(value) || !/^\d+(|\.\d+)$/.test(value) || value > max || value < min) && callback(new Error(`只能为${min}-${max}`))
            callback()
        }
    },
     ...
     * 数组必须有一个值
     */
    array: function (rule, value, callback) {
        //检验位的检测
        if (!value.length) {
     ...
     * 必须不能是汉字>0
     */
    cannottChinese: function (rule, value, callback) {
        //检验位的检测
        if (/[\u4e00-\u9fa5]/g.test(value)) {
            callback(new Error('必须是数字字母组合'))
        }
        callback()
    },
