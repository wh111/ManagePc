            ...
            <el-form ref="formValidate" :model="formValidate" :rules="testPaperManagementZn0" label-width="100px"
                ...
                <el-form-item label="题型：" prop="questionType">
                    <el-checkbox-group v-model="formValidate.questionType">
                        <el-checkbox v-for="(item,index) in questionsTypeArr" :key="index" :label="item.id">
                            {{ item.name }}
                        </el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
              ...
              <el-form-item label="试卷结构：" class="paperConstrutor">
                    <el-table align="center" :context="self" class="questionKnowledgeTable" maxHeight="400"
                        ...
                        <el-table-column label="操作" align="center" width="70">
                            <template slot-scope="scope">
                                <el-button size="small" type="danger" @click="removeQuestions(scope.$index)">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                </el-form-item>
    ...
    methods: {
      ...
      changeHeader (obj, select) {
        for (let key in this.headerKey) {
          if (key !== 'textbookVersionId' && this.formValidate[key] !== '' && this.formValidate[key] != select[key]) {
            this.changeGradeOrTerm = true // 选择的信息被修改过（验证使用）
            console.log('改变过', key, this.formValidate[key], select[key])
          }
      ...
      * */
      ...
      getPreviewData () {
        ...
        data.papsersQuestionsTypeScoreList = questionsTypeScore
      ...
      getKnowledgeDataForServer () {
        this.getDataForServer('knowledge', {chapterIds: this.selectFormValidate.selectChapter.join(',')}, (data, res) => {
          let thisKDId = []
          let selectknowledge = this.selectFormValidate.selectknowledge
          let knowledgeIds = Object.keys(data)
