            ...
            <div>
                <el-form :rules="rules" ref="formValidate" :model="formValidate" class="demo-ruleForm"
                  ...
                  <el-col :span="14" style="margin-bottom:20px;margin-top: 10px">
                      <el-button size="small" class="greenButton" @click="add">新增人员</el-button>
                      <el-button size="small" class="orangeButton" @click="reset">默认密码</el-button>
                    </el-col>
                </el-form>
            </div>
            ...
            <div id="myTable" ref="myTable">
                <el-table sortable="custom" @sort-change="sortChange" align="center" :context="self" :height="dynamicHt"
                  ...
                  <el-table-column prop="index" align="center" label="序号" width="70px"
                    ...
                    <el-table-column prop="roleName" label="权限" show-overflow-tooltip align="center">
                    </el-table-column>
                    <el-table-column prop="lastLoginTime" label="登录时间" show-overflow-tooltip align="center">
                        <template slot-scope="scope">
                            {{scope.row.lastLoginTime | formatDate('yyyy-MM-dd HH:mm')}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="accountStatus" label="状态" show-overflow-tooltip align="center">
                        <template slot-scope="scope">
                    ...
                    <el-table-column label="操作" align="center" width="280">
                        <template slot-scope="scope">
                          <el-button :disabled="scope.row.accountStatus == 1" size="small" class="tableMakeItem"
                                     @click="edit(scope.row)">修改
                          </el-button>
                          <el-button :disabled="scope.row.self" v-if="scope.row.accountStatus == '0'" size="small"
                                     class="tableMakeItem"
                                       @click="enable(scope.row,'1')">冻结
                            </el-button>
                          <el-button v-else size="small" class="tableMakeItem" @click="enable(scope.row,'0')">恢复
                          </el-button>
                        </template>
                    </el-table-column>
                </el-table>
...
<script>
    import add from './adminManage_add.vue'
    import edit from './adminManage_edit.vue'
    import api from './api'
    import {
        list as rules
    } from './rules'
    ...
    export default {
        ...
        methods: {
            ...
            reset() {
                ...
                Util.todoConfirm({
                    title: '确定重置用户密码为默认密码吗？',
                    content: {id: 'todoId', title: '重置用户密码'},
                    data: {
                        data: {ids: Util.getStrByArr(this.multipleSelection, 'id')},
                        api: api.setDefaultPassword,
                        msg: '重置用户密码'
                    },
                    close: () => {//关闭后触发
                        this.subCallback()
